export default function initFooter(footerData) {
  class FooterType1 extends HTMLElement {
    ...
    render(footer) {
      const socialsHTML = footer.socials.socials_names
        ? footer.socials.socials_names
            .map((social) => {
              if (this.logoRender(social)) {
                return `<a href="https://${social}" target="_blank">${this.logoRender(
                  social
                )}</a>`;
              }
            })
  ...
  class FooterType3 extends HTMLElement {
    ...
    render({ title, description, footerItems, socials }) {
      ...
      const socialsHTML = socials.socials_names
        ? socials.socials_names
            .map((social) => {
              if (this.logoRender(social)) {
                return `<a href="https://${social}" target="_blank">${this.logoRender(
                  social
          ...
          <div>
            ...
            <ul class="space-y-3 mt-4">
              ${item2.items
                .map(
                  (item, i) =>
                    `<li class="footer-item hover:underline hover:underline"><a href=${item2.links[i]}>${item}</a></li>`
                )
                .join("")}
            </ul>
          </div>
  ...
  function logoRender(logosData) {
    let toLowerCasedLogosData = logosData.toLowerCase();
    switch (true) {
      case toLowerCasedLogosData.includes("instagram"):
        return `<svg xmlns="http://www.w3.org/2000/svg" class="footer-icon" fill="currentColor" viewBox="0 0 16 16">
      ...
      case toLowerCasedLogosData.includes("facebook"):
        return `<svg xmlns="http://www.w3.org/2000/svg" class="footer-icon" fill="currentColor" class="bi bi-facebook" viewBox="0 0 16 16">
              <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951"/>
              </svg>`;
      case toLowerCasedLogosData.includes("x.com"):
        return `<svg xmlns="http://www.w3.org/2000/svg" class="footer-icon" fill="currentColor" class="bi bi-twitter-x" viewBox="0 0 16 16">
               <path d="M12.6.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867-5.07-4.425 5.07H.316l5.733-6.57L0 .75h5.063l3.495 4.633L12.601.75Zm-.86 13.028h1.36L4.323 2.145H2.865z"/>
              </svg>`;
      case toLowerCasedLogosData.includes("t.me"):
        return `<svg xmlns="http://www.w3.org/2000/svg" class="footer-icon" fill="currentColor" class="bi bi-telegram" viewBox="0 0 16 16">
              <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8.287 5.906q-1.168.486-4.666 2.01-.567.225-.595.442c-.03.243.275.339.69.47l.175.055c.408.133.958.288 1.243.294q.39.01.868-.32 3.269-2.206 3.374-2.23c.05-.012.12-.026.166.016s.042.12.037.141c-.03.129-1.227 1.241-1.846 1.817-.193.18-.33.307-.358.336a8 8 0 0 1-.188.186c-.38.366-.664.64.015 1.088.327.216.589.393.85.571.284.194.568.387.936.629q.14.092.27.187c.331.236.63.448.997.414.214-.02.435-.22.547-.82.265-1.417.786-4.486.906-5.751a1.4 1.4 0 0 0-.013-.315.34.34 0 0 0-.114-.217.53.53 0 0 0-.31-.093c-.3.005-.763.166-2.984 1.09"/>
              </svg>`;
      case toLowerCasedLogosData.includes("youtube"):
        return `<svg xmlns="http://www.w3.org/2000/svg" class="footer-icon" fill="currentColor" class="bi bi-youtube" viewBox="0 0 16 16">
      ...
      case toLowerCasedLogosData.includes("whatsapp"):
        return `<svg xmlns="http://www.w3.org/2000/svg" class="footer-icon" fill="currentColor" class="bi bi-whatsapp" viewBox="0 0 16 16">
      ...
      case toLowerCasedLogosData.includes("linkedin"):
        return `<svg xmlns="http://www.w3.org/2000/svg" class="footer-icon" fill="currentColor" class="bi bi-linkedin" viewBox="0 0 16 16">
